...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        ...
        await altron.edit(f"𝐊ʏα 𝐎яƉɛя 𝐇αι MALIK")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __ᴀᴅᴅɪɴɢ SPAM ʙᴏᴛ sᴜᴅᴏ....__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"ᴛʜɪꜱ ᴜꜱᴇʀ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴀ  sᴘᴀᴍ ʙᴏᴛ ꜱᴜᴅᴏ ᴜꜱᴇʀ !!")
        else:
            if len(sudousers) > 0:
                newsudo = f"{sudousers} {target}"
            else:
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**: `{target}`\n» `ᴡᴀɪᴛ ᴋᴀʀ ʙʜᴀɪ BOT sᴜʀᴜ ʜᴏ ʀʜᴀ ʜᴀɪ...`")
            heroku_var["SUDO_USERS"] = newsudo    
    
    elif event.sender_id in SUDO_USERS:
        await event.reply("» AUKAT ME REH MADHARCHOD ")
